...
def rescale_img(tensor):
    # Rescale the tensor image to [0, 1] based on its own min and max values
    min_val = tensor.min()
    max_val = tensor.max()
...
combined_flip_transform = torchvision.transforms.Compose([hflip, vflip])
normalize = torchvision.transforms.Normalize([0.5], [0.5])
def transform_augment(img_list, split='val', min_max=(0, 1)):    
    imgs = [rescale_img(totensor(img)) for img in img_list]
    if split == 'train':
        imgs = torch.stack(imgs, 0)
        imgs = combined_flip_transform(imgs)
        imgs = torch.unbind(imgs, dim=0)
    ret_img = [normalize(img) for img in imgs]
    return ret_img
